const ReportsContent = ({ isNightMode, onNightModeToggle, onFullScreenToggle }) => {
    const [isModalOpen, setIsModalOpen] = React.useState(false);
    const [selectedReport, setSelectedReport] = React.useState(null);

    const reports = {
        "Events & Performance": [
            "Incident Hotspot Analysis (by Time & Location)",
        ...
        "Administrative & Operations": [
            ...
            "Controlled Substance Usage & Inventory Log",
            "Exposure Report Tracking"
        ]
    };
    ...
    const ReportParameterModal = () => (
        <Modal onClose={() => setIsModalOpen(false)}>
            ...
            <div style={{marginTop: '15px'}}>
                <SelectField label="Filter by Unit (Optional)">
            ...
            <div style={{display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '30px', borderTop: '1px solid var(--light-gray)', paddingTop: '20px' }}>
                 ...
                 <button onClick={() => setIsModalOpen(false)} style={{ background: 'var(--primary)', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>Run Report</button>
            </div>
        </Modal>
    );
